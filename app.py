...
if cursor.fetchone()[0] == 0:
    cursor.execute("INSERT OR IGNORE INTO friends (username, friend_name) VALUES (?, ?)", ("AdminMike1", "User_Delta"))
    cursor.execute("INSERT OR IGNORE INTO friends (username, friend_name) VALUES (?, ?)", ("AdminMike1", "NewBuddy99"))
    cursor.execute("INSERT OR IGNORE INTO requests (username, requester) VALUES (?, ?)", ("AdminMike1", "User_Delta"))
    conn.commit()

# Helper fetch functions
def get_friends():
    cursor.execute("SELECT friend_name FROM friends WHERE username = ?", (st.session_state.username,))
    return [row[0] for row in cursor.fetchall()]

def get_requests():
    cursor.execute("SELECT requester FROM requests WHERE username = ?", (st.session_state.username,))
    return [row[0] for row in cursor.fetchall()]
...
if "groups" not in st.session_state:
    st.session_state.groups = {
        "General Chat": {"owner": "System", "members": ["AdminMike1", "User_Delta"], "banned": []},
        "WorkSpace": {"owner": "AdminMike1", "members": ["AdminMike1", "User_Delta"], "banned": []}
    }

# Standard list of common swear words to filter when Safe Chat is on
BAD_WORDS = ["shit", "fuck", "bitch", "ass", "damn", "crap", "bastard", "dick", "piss", "slut", "whore", "spam"]
...
with st.sidebar:
    ...
    with st.expander("⚙️ Edit Profile"):
        ...
        st.session_state.safe_chat = st.checkbox("🛡️ Safe Chat Filter", value=st.session_state.safe_chat)
        st.session_state.simulate_network_error = st.checkbox("🔌 Simulate Server/Net Error", value=st.session_state.simulate_network_error)
        ...
        if not current_requests:
            ...
            for req in list(current_requests):
                ...
                if col_r1.button("Accept", key=f"acc_{req}"):
                    cursor.execute("INSERT OR IGNORE INTO friends (username, friend_name) VALUES (?, ?)", (st.session_state.username, req))
                    cursor.execute("DELETE FROM requests WHERE username = ? AND requester = ?", (st.session_state.username, req))
                    conn.commit()
                    st.rerun()
    ...
    search_filter = st.text_input("Filter list...", placeholder="Filter list...", label_visibility="collapsed")
    ...
    for friend in current_friends:
        if friend not in st.session_state.blocked_users:
            if search_filter.lower() in friend.lower():
                ...
                if st.button(f"{icon}  {friend}", key=f"dm_{friend}", use_container_width=True):
    ...
    for group in st.session_state.groups:
        if group not in st.session_state.blocked_users:
            if search_filter.lower() in group.lower():
                ...
                icon = "🔴" if is_active else "⚪"
                if st.button(f"{icon}  #{group}", key=f"grp_{group}", use_container_width=True):
                    st.session_state.current_channel = group
                    st.rerun()

# --- MAIN CONTENT AREA ---
st.title(f"{'#' if st.session_state.current_channel in st.session_state.groups else '💬'} {st.session_state.current_channel}")
...
if curr_group and curr_group["owner"] == st.session_state.username:
    with st.expander("🛡️ Group Owner Controls & Settings"):
        renamed_input = st.text_input("Rename Group Chat", value=st.session_state.current_channel)
        if st.button("Save New Name"):
            old_name = st.session_state.current_channel
            if renamed_input and renamed_input != old_name and renamed_input not in st.session_state.groups:
                st.session_state.groups[renamed_input] = st.session_state.groups.pop(old_name)
                # Update channel name in DB table as well
                cursor.execute("UPDATE messages SET channel = ? WHERE channel = ?", (renamed_input, old_name))
                conn.commit()
        ...
        st.markdown("---")
        member_to_ban = st.selectbox("Ban Member", [m for m in curr_group["members"] if m != st.session_state.username])
    ...
    if u == st.session_state.username:
        ...
        if col_act1.button("💬 Talk"):
            if u in current_friends or u in st.session_state.groups:
                ...
                cursor.execute("INSERT OR IGNORE INTO requests (username, requester) VALUES (?, ?)", (u, st.session_state.username))
                conn.commit()
                st.warning(f"Connection request sent to {u}! Waiting for them to accept...")
        ...
        if col_act3.button("🗑️ Delete Contact"):
            cursor.execute("DELETE FROM friends WHERE username = ? AND friend_name = ?", (st.session_state.username, u))
            conn.commit()
            st.session_state.inspect_user = None
...
# Custom WebRTC Call Interface with Working Mute & Camera Controls
if st.session_state.in_call:
    ...
    with cc2:
        cam_label = "📷 Turn Cam On" if st.session_state.cam_off else "🚫 Turn Cam Off"
    ...
    webrtc_streamer(
        ...
        media_stream_constraints={
            "audio": False if st.session_state.is_muted else True,
...
with chat_container:
    ...
    if not channel_msgs:
        st.info(f"No messages in {st.session_state.current_channel} yet. Say hi below!")
    else:
        for idx, msg in enumerate(channel_msgs):
            col_msg_avatar, col_msg_body = st.columns([1, 15])
            with col_msg_avatar:
                if st.button("👤", key=f"prof_{idx}_{msg['user']}={msg['time']}"):
...
with st.form(key="message_form", clear_on_submit=True):
    ...
    if submit_btn and user_input:
        ...
        if is_banned:
            ...
            # Save message to database so all devices can fetch it
            cursor.execute(
                "INSERT INTO messages (channel, user, text, time) VALUES (?, ?, ?, ?)",
                (st.session_state.current_channel, st.session_state.username, cleaned_text, timestamp)
            )
            conn.commit()
            st.rerun()